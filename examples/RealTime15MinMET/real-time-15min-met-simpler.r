...
get.archive.data <- function() {
  ...
  ddg.procedure(params=list(archive.file))
...
get.current.data <- function() {
  ...
  ddg.procedure(params=list("current.url"))
...
get.final.data <- function(ad,cd) {
  ...
  ddg.procedure(params=list("archive-data.csv", "current-data.csv"))
  # Cannot create the following 2 edge automatically.  It looks for the arguments passed
  # to get.final.data, which are archive-data and current-data, without the .csv.
...
save.data <- function(fn,xx) {
  ...
  ddg.procedure(params=list(fn))
  # Cannot create this edge automatically.  Looking for final.data.
...
plot.data <- function(zz,v,d,output) {
  ...
  if (output=="gui") {
	  # Including function name as string for testing purposes
    ddg.procedure("plot.data", params=list("final-data.csv",deparse(substitute(v)), deparse(substitute(d))))
	# Cannot create this edge.  Looking for final-data.
	#ddg.data.in("plot.data","final-data.csv")
    #ddg.data.in("plot.data",v)
    #ddg.data.in("plot.data",d)
	# Wants to create a data-in edge from "gui".  No such node.
  ...
  if (output=="file") {
    dev.off()
	# Including function name as name for testing purposes
    ddg.procedure(plot.data, params=list("final-data.csv",deparse(substitute(v)), deparse(substitute(d))))
	# Cannot create this edge.  Looking for final-data.
	#ddg.data.in("plot.data","final-data.csv")
    #ddg.data.in("plot.data","variable")
    #ddg.data.in("plot.data","days")
	# Wants to create a data-in edge from "file".  No such node.
