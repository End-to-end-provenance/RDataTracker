...
#plot.file <- plot.data(raw.data,"R")
plot.file <- plot.data(raw.data)

ddg.debug.on()
ddg.finish("Read data")
ddg.file(data.file)
ddg.data.in(eval(data.file), pname="raw.data <- read.data(data.file, start.date, end.date, varia")
ddg.file.out(plot.file, pname="plot.file <- plot.data(raw.data)")
ddg.debug.off()
...
ddg.restore(qc.checkpoint)

gap.fill.parameters <- read.csv("par-gf2.csv")
gap.filled.data <- gap.fill(quality.controlled.data, gap.fill.parameters)
plot.data(gap.filled.data,"G")

# ddg.restore(qc.checkpoint)
# 
# gap.fill.parameters <- read.csv("par-gf3.csv")
# gap.filled.data <- gap.fill(quality.controlled.data, gap.fill.parameters)
# plot.data(gap.filled.data,"G")
