...
ddg.init(
		"/Users/barbaralerner/Documents/Process/DataProvenance/workspace/ddg-r/examples/DailySolarRadiation/daily-solar-radiation-minimal.r",
...
#  variable <<- "slrt"
  ...
  ddg.procedure(pname="read.csv", ins=list("data.file"), outs=list("zz"))
  zz$date <- as.Date(zz$date)
  all.data <<- subset(zz,zz$date>=start.date & zz$date<=end.date)
  ddg.procedure(pname="subset dates", ins=list("zz", "start.date", "end.date"), outs=list("all.data"))
  
  raw.data <<- all.data[c("date",variable)]
  names(raw.data)[names(raw.data)==variable] <<- "raw"
  data.rows <<- nrow(raw.data)
  ddg.procedure(pname="subset variable", ins=list("all.data", "variable"), outs=list("raw.data"))
...
#  gap.fill.parameters <<- read.csv(gf.file)
  ...
  # RawData data frame
	#ddg.procedure(pname="initialize data frame", ins=list("raw.data"))
	raw.data$cal <<- 0
  raw.data$cal.f <<- ""
  raw.data$qc <<- 0
  raw.data$qc.f <<- ""
  raw.data$gf <<- 0
  raw.data$gf.f <<- ""
  ddg.procedure(pname="initialize data frame", ins=list("raw.data"), outs=list("raw.data"))
  #ddg.data.out("raw.data", pname="initialize data frame")
...
variable <- "slrt"
#ddg.procedure(pname="initialization", outs=list("data.file", "start.date", "end.date", "variable"))
ddg.procedure(pname="initialization")
ddg.data.out(pname="initialization","data.file")
ddg.data.out(pname="initialization","start.date")
ddg.data.out(pname="initialization","end.date")
ddg.data.out(pname="initialization","variable")
