...
ddg.init("/Users/barbaralerner/Documents/Process/DataProvenance/workspace/ddg-r/examples/S4ObjectTest/S4ObjectTest.R",
		 "/Users/barbaralerner/Documents/Process/DataProvenance/workspace/ddg-r/examples/S4ObjectTest/ddg",
		enable.console = TRUE)
...
generateSamples <- function (n) {
	ddg.start()
	samplesArr <- list()
	ddg.procedure("new samples", outs=list("samplesArr"))
	for (ix in 1:n) {
		newSample <- new ("SampleObj", 
				sampleCode = ix, 
				nIndividuals = as.integer(ix * 10), 
				areaCode = as.integer(ix * 100))
		ddg.procedure("new sample", outs=list("newSample"))
