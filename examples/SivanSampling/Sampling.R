...
addToDictionary <- function (speciesDistributionAreas, speciesStr) {
	...
	ddg.procedure("add species to dictionary",
			ins=list("probabilityStr"),
			outs=list("speciesDistributionAreas"))
...
assignSizeDistPropertyFun <- function(object, inSampleSizeDist) {
	...
	ddg.procedure(pname="bind object@sampleSizeDistObj", 
			...
			outs=list("object@sampleSizeDistObj"))
...
defineSamplesSizeDistribution <- function() {
	...
	ddg.procedure(outs=list("sampleSizeDistributionMng"))
...
assignSampleData <- function(sampleInAreaStr, areaCode, speciesDistribution, sampleSizeDistributionMng) {
	...
	#ddg.procedure(lookup.ins=TRUE, outs=list("samplesArr[sampleCode][[1]]"))
}
...
assignSamplesToArea <- function(areaStr, speciesDistribution, sampleSizeDistributionMng) {
	...
	if (length(areaStr > 0)) {
		...
		ddg.procedure(pname="extract tmpSamplesInArea", 
				ins=list("areaStr"), 
				outs=list("tmpSamplesInArea"))
...
writeToFile <- function(sample, smplx, fileConn, totalNumOfSample) {
	...
	ddg.data.out(smpCompositions, pname="raffleIndividualsPerSample")
