...
# at the time of execution and their version numbers.
...
# .ddg.installedpackages.json returns information on installed
# packages and version numbers for the JSON file.
...
.ddg.installedpackages.json <- function() {
  ...
  if (nrow(installed) == 0) {
    ...
    output <- paste(output, packagearray, " ]\n", sep = "")
  }
  return(output)
}
...
.ddg.json.environ <- function() {
  ...
  environ <- paste(environ, .ddg.json.nv("rdt:script", script), sep="")
...
ddg.source <- function (file,  ddgdir = NULL, local = FALSE, echo = verbose, print.eval = echo,
  ...
  # Store script number & name. Number of main script is 0.
