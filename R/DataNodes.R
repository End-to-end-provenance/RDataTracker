...
.ddg.init.data.nodes <- function () {
  ...
  # Set snapshots and max.snapshot.size.  Make sure they are not already set, 
  # as someone may have called prov.set.detail.
  
  if (!.ddg.is.set("ddg.snapshots")) {
    .ddg.set("ddg.snapshots", FALSE)
  }
...
#' .ddg.snapshots returns True if snapshots are being saved
#' @return True if snapshots are being saved
#' @noRd

.ddg.snapshots <- function() {
  return(.ddg.get("ddg.snapshots"))
}
...
#' files by setting the parameters snapshots and max.snapshot.size
...
.ddg.snapshot.node <- function(dname, fext, data, save.object = FALSE, 
  ...
  # Determine if snapshots should be saved
  snapshots <- .ddg.snapshots()
  ...
  if (max.snapshot.size == Inf || utils::object.size(data) < max.snapshot.size * 1024) {
    full.snapshot <- TRUE
  }
